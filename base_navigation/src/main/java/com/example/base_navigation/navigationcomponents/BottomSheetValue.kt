...
class BottomSheetState internal constructor(
    ...
    init {
        if (skipHalfExpanded) {
            require(initialValue != BottomSheetValue.HalfExpanded) {
    ...
    val isVisible: Boolean
        get() = swipeableState.currentValue != BottomSheetValue.Hidden
    ...
    val hasHalfExpandedState: Boolean
        get() = swipeableState.hasAnchorForValue(BottomSheetValue.HalfExpanded)
     ...
     */
    suspend fun expand() {
        if (!swipeableState.hasAnchorForValue(BottomSheetValue.Expanded)) {
            return
        }
    ...
    ) = animateTo(BottomSheetValue.Hidden, swipePriority = swipePriority)
    ...
    BoxWithConstraints(modifier.fillMaxSize()) {
        ...
        Box(
            Modifier
                ...
                .swipeAnchors(
                    state = sheetState.swipeableState,
                    possibleValues = setOf(BottomSheetValue.Hidden,
                        BottomSheetValue.HalfExpanded, BottomSheetValue.Expanded
                    ),
                    anchorChangeHandler = anchorChangeHandler
                ) { state, sheetSize ->
                    when (state) {
                        BottomSheetValue.Hidden -> fullHeight
                        BottomSheetValue.HalfExpanded -> when {
                            sheetSize.height < fullHeight / 2f -> null
                            sheetState.skipHalfExpanded -> null
                            else -> fullHeight / 2f
                        }

                        BottomSheetValue.Expanded -> if (sheetSize.height != 0) {
                ...
                .semantics {
                    if (sheetState.isVisible) {
                        dismiss {
                            if (sheetState.swipeableState.confirmValueChange(BottomSheetValue.Hidden)) {
                                onDismissRequest()
                            }
                            true
                        }
                        if (sheetState.swipeableState.currentValue == BottomSheetValue.HalfExpanded) {
                            expand {
                                if (sheetState.swipeableState.confirmValueChange(BottomSheetValue.Expanded)) {
                            ...
                            collapse {
                                if (sheetState.swipeableState.confirmValueChange(BottomSheetValue.HalfExpanded)) {
...
private fun ModalBottomSheetAnchorChangeHandler(
    ...
    val newTarget = when (previousTarget) {
        ...
        BottomSheetValue.HalfExpanded -> {
            when {
                newAnchors.containsKey(BottomSheetValue.HalfExpanded) -> BottomSheetValue.HalfExpanded
                newAnchors.containsKey(BottomSheetValue.Expanded) -> BottomSheetValue.Expanded
