 ...
 *
 * If you want to allow [NavBackHandler] to remove the last entry as well, you may set
 * [allowEmptyBackstack] to `true`. For example, `allowEmptyBackstack = true` should be used in
 * pair with `BottomSheetNavHost`.
 *
 ...
 * the same reason that is described in the notice above).
 */
@Composable
fun <T> NavBackHandler(
    controller: NavController<T>,
    enabled: Boolean = true,
    allowEmptyBackstack: Boolean = false
) = BackHandler(
    enabled = enabled && controller.backstack.entries.size > if (allowEmptyBackstack) 0 else 1
) {
    controller.pop()
}